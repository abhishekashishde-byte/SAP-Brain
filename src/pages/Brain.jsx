...
export default function Brain({ session }) {
  ...
  const handleImprovePrompt = async () => {
    ...
    try {
      ...
      if (result?.suggest === true && result?.suggestion) {
        ...
        setKnowledgeToast('Your question is already clear enough.')
        setTimeout(() => setKnowledgeToast(null), 2200)
      }
    } catch {
      setKnowledgeToast('Could not improve the question right now.')
      setTimeout(() => setKnowledgeToast(null), 2200)
    } finally {
      setIsImprovingPrompt(false)
    }
  ...
  const handleSend = async (overrideText) => {
    ...
    setPendingPromptSuggestion(null)
    return submitQuestion(safeOverride)
  }
  ...
  const addExactWaniBranding = async (imageBase64, themeKey = activeThemeKey) => {
    ...
    const waniMark = await loadImage('/wani-handout-mark.svg')
    ...
    const gap = Math.round(footerH * 0.14)
    const margin = Math.round(footerH * 0.32)
  ...
  const handleGenerateHandout = async (convId, msgIndex, questionText, answerText) => {
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ action: 'generate_handout', question: questionText, answerText, messages: (messages || []).filter(m => m.role === 'user' || m.role === 'assistant').map(m => ({ role:m.role, content:m.content })), themeKey: activeThemeKey }),
    })
  ...
  const handleGenerateVisual = async (convId, msgIndex, questionText, answerText) => {
    ...
    const res = await fetch('/api/chat', {
      ...
      body: JSON.stringify({ action: 'generate_visual', question: questionText, answerText, messages: (messages || []).filter(m => m.role === 'user' || m.role === 'assistant').map(m => ({ role:m.role, content:m.content })), themeKey: activeThemeKey }),
    })
    const data = await res.json().catch(() => ({}))
    ...
    <div style={{ display:'flex',width:'100%',height:'100dvh',background:t.bg,fontFamily:"'Inter','DM Sans',sans-serif",overflow:'hidden',overflowX:'hidden' }}>
            ...
            <div className="chat-input-wrap" style={{ borderTop:`1px solid ${t.border}`,background:t.topbar,backdropFilter:'blur(10px)',flexShrink:0,position:'relative',zIndex:2,paddingBottom:'calc(env(safe-area-inset-bottom, 0px) + 10px)' }}>
              ...
              <div style={{ maxWidth:720,margin:'0 auto' }}>
                ...
                >
                  ...
                  <button onClick={handleImprovePrompt}
      ...
      {showUpdateNotice && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.55)', zIndex:500, display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
          <div style={{ width:'min(460px,94vw)', background:t.surface, border:`1px solid ${t.border}`, borderRadius:18, padding:22, boxShadow:'0 20px 60px rgba(0,0,0,0.35)' }}>
            ...
            <div style={{ fontSize:13, color:t.text3, lineHeight:1.6, marginBottom:14 }}>You can now ask Wani to improve a question before sending it.</div>
            <div style={{ fontSize:13, color:t.text2, lineHeight:1.7, marginBottom:18 }}>
              <div>• Click ✨ only when you want help improving your prompt.</div>
              <div>• Wani never interrupts your typing or checks every question automatically.</div>
      ...
      {pendingPromptSuggestion && (
        <div style={{ position:'fixed', left:'50%', bottom:isMobile?82:92, transform:'translateX(-50%)', zIndex:260, width:'min(620px,92vw)', background:t.surface, border:'1px solid rgba(79,70,229,0.38)', borderRadius:14, padding:'14px 16px', boxShadow:'0 10px 36px rgba(0,0,0,0.28)' }}>
          ...
          <div style={{ display:'flex', gap:8 }}>
            <button onClick={()=>setPendingPromptSuggestion(null)} style={{ flex:1, padding:'8px 10px', borderRadius:9, border:`1px solid ${t.border}`, background:'transparent', color:t.text2, cursor:'pointer', fontWeight:600 }}>Keep original</button>
            <button onClick={()=>{ setInput(pendingPromptSuggestion.suggestion); setPendingPromptSuggestion(null); setTimeout(()=>inputRef.current?.focus(),0) }} style={{ flex:1, padding:'8px 10px', borderRadius:9, border:0, background:'#4F46E5', color:'#fff', cursor:'pointer', fontWeight:650 }}>Use suggestion</button>
          </div>
        </div>
      )}

      {isImprovingPrompt && (
        <div style={{ position:'fixed', bottom:isMobile?82:92, left:'50%', transform:'translateX(-50%)', zIndex:250, fontSize:11, color:t.text3, background:t.surface, border:`1px solid ${t.border}`, borderRadius:20, padding:'5px 10px' }}>Improving question…</div>
      )}
