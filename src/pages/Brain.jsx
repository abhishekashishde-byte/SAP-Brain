...
export default function Brain({ session }) {
  ...
  const handleSend = async () => {
    ...
    try {
      const res = await fetch('/api/chat', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({ messages:currentMsgs, module:currentMod, topic:currentTopic, tone }) })
              ...
              <div style={{ maxWidth:720, margin:'0 auto' }}>
                {messages.length === 0 ? (
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'calc(100vh - 280px)', textAlign:'center', animation:'fadeIn 0.4s ease' }}>
                    ...
                    <div style={{ fontFamily:"'Playfair Display',serif", fontSize:22, fontWeight:600, color:t.text, marginTop:14, marginBottom:8 }}>
                      {browseTopic || browseModule?.split('–')[0].trim() || 'Ask Wani'}
                    </div>
                    <p style={{ fontSize:13, color:t.text3, maxWidth:300, lineHeight:1.7, marginBottom:22 }}>
