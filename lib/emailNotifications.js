...
export async function notifySignup(client, user) {
  ...
  return sendOnce(client, {
    ...
    send: () => sendEmail({
      ...
      html: emailFrame(
        ...
        `<p><strong>${escapeHtml(name)}</strong> has created a Wani account.</p><p style="line-height:1.7"><strong>Email:</strong> ${escapeHtml(email)}<br><strong>User ID:</strong> ${escapeHtml(user.id)}</p>`,
...
export async function notifyLogin(client, user, sessionId) {
  ...
  return sendOnce(client, {
    ...
    send: () => sendEmail({
      ...
      text: `${name} (${email}) logged in to Wani.`,
      html: emailFrame('User logged in', `<p><strong>${escapeHtml(name)}</strong> logged in to Wani.</p><p><strong>Email:</strong> ${escapeHtml(email)}</p>`),
    }),
  })
}
