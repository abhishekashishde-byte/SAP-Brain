...
export async function handleAdminDashboard(res, auth) {
  ...
  // Keep the admin dashboard intentionally lightweight. The old implementation
  // downloaded every conversation's full `messages` JSON on every refresh just
  // to count generated visuals. As conversation history grew, that query began
  // hitting Supabase's statement timeout and then Vercel's function timeout.
  // Credits are only needed for today's/month's dashboard figures, so query only
  // the current month instead of the entire usage history.
  const [usersResult, profilesResult, approvedResult, sessionsResult, creditsResult] = await Promise.all([
    ...
    client.from('wani_credit_usage').select('user_id,created_at').gte('created_at', monthStart.toISOString()),
  ])
  ...
  for (const row of credits) {
    const item = usageByUser.get(row.user_id) || { today: 0, month: 0, lastQuestionAt: null }
  ...
  const rows = users.map(user => {
    ...
    const usage = usageByUser.get(user.id) || { today: 0, month: 0, lastQuestionAt: null }
  ...
  return res.status(200).json({
    ...
    telemetry: {
      ...
      note: 'Conversation-image totals are temporarily excluded from the live dashboard because scanning full conversation payloads made the dashboard time out. They should be moved to lightweight persisted telemetry before being re-enabled.',
    },
    users: rows,
  })
}
